...
<script>
	...
	export default {
		...
		methods: {
			...
			setTransition(route) {
				...
				if(hasRoutes.length === 0) {
					...
					this.$_jQuery(".header-nav .hover-box").stop(true, true).animate({
						'right': (this.mainRouters.length - 1 - num) * this.headMenuWidth + 14
					}, 300);
				}
			},
			updateUI() {
        ...
        updUserPass(userinfo) {
				...
				dealForecast(predRes){//处理预报的预警数据
					let tableData=[];
					let provinceCode=this.config.provinceCode;
					for(let i=0;i<predRes.length;i++){
						let obj={};
						obj['type']=predRes[i].warningcitycode===provinceCode?"省级":"市级";
						obj['warnTime']=predRes[i]['warningdate'].split("T")[0];
						obj['timeRange']=predRes[i]['starttime'].split("T")[0]+' 至 '+predRes[i]['endtime'].split("T")[0];
						obj['level']=this.setLevel(predRes[i]['warninglevel']).level;
						obj['levelColor']=this.setLevel(predRes[i]['warninglevel']).color;
						obj['primarypollutant']=predRes[i]['primarypollutant']?utils.addSubToLabel(predRes[i]['primarypollutant'].toUpperCase()):"-";
				...
				dealMonitor(res){//处理实况的预警数据
					let arr=[];
					for(let i=0;i<res.length;i++){
						let obj={};
						let sTime=dateUtils.strToDate(res[i]['starttime'].replace("T"," "));
						let eTime=dateUtils.strToDate(res[i]['endtime'].replace("T"," "));
						obj['timeRange']=dateUtils.dateToStr("YYYY-MM-DD HH时",sTime)+' 至 '+dateUtils.dateToStr("YYYY-MM-DD HH时",eTime);
						obj['cityname']=[res[i]['warningcity']];
						obj['primarypollutant']=res[i]['primarypollutant']?(utils.addSubToLabel(res[i]['primarypollutant'])):"-";
						obj['warnTime']=dateUtils.dateToStr("YYYY-MM-DD HH时",dateUtils.strToDate(res[i]['warningdate'].replace("T"," ")));
...
<style scoped>
	...
	.back-div{
		transform:skew(-30deg);bottom:0px;right:14px;height:38px;background-color:#00ad94;position:absolute;
	}
	.hover-box {
		position: absolute;
