<template lang="html">
	<div class="content left-nav" :style="{'width':contentWidth,'top':headerHt+5+'px','height':win_size.height}">
		<div class="top">
...
<script>
	...
	export default {
		...
		methods: {
			setSize() {
				this.win_size.height = "calc(" + this.$_jQuery(window).height() + "px - " + (this.headerHt + 5) + "px)";
			...
			setTranistion(item, parent) {
				...
				this.$_jQuery(".left-nav .hover-box").stop(true, true).animate({
					'top': num * 50
				}, 300);
			},
			getColors(idx) {
			...
			hasTwoLevel() {
				...
				for(let i = 0; i < this.$router.options.routes.length; i++) {
					...
					if(this.$route.matched[0].path === item.path && item.children && item.children.length > 0) {
						for(let x = 0; x < item.children.length; x++) {
							if(item.children[x].children && item.children[x].children.length > 0) {
								has = true;
								break
							}
						}
			...
			isActived(headerItem,oneLevel,twoLevel){
				if(twoLevel){//判断二级菜单当前选中
					if(headerItem.path+"/"+oneLevel.path+"/"+twoLevel.path===this.$store.state.router.leftCurRouter){
						return true
					}else{
						return false
					}
				}else{//判断一级菜单当前选中
					if(headerItem.path+"/"+oneLevel.path===this.$store.state.router.leftCurRouter&&!oneLevel.children){
						return true
					}else{
						return false
					}
